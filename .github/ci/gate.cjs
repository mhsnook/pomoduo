...
//
// `bundle` starts report-only. The repo has never watched these numbers, so
// nobody yet knows what an ordinary PR moves. Watch the reported delta for a
// few weeks, then swap in `{ maxGzDelta: '20KiB' }` once a normal week is known.
const POLICY = {
	build: 'no-new',
	typecheck: 'no-new',
	lint: 'no-new',
